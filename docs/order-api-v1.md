...
キッチン管理の「営業時間」画面から、商品ジャンルごとの提供曜日と提供時間を編集します。保存先は会員証・モバイルオーダー側の共通メニューDBで、モバイルオーダーとセルフレジは同じ設定を参照します。

- キッチン側は認証済み画面から `GET/PUT /api/v1/kitchen/menu-availability` を呼ぶ
- キッチンAPIは共通メニュー側の `GET/PUT /api/v1/kitchen/category-schedules` へ `KITCHEN_API_TOKEN` 付きで転送する
- 店舗休業中はジャンル設定に関係なく注文不可とする
- 商品個別の販売時間が設定されている場合は、個別設定を優先する
- 初期運用では「ご飯もの」を土日の11:30〜21:30だけ提供する。丼に加えて、名称に「ご飯」または「ライス」を含む単品商品も同じ制限へ従う

## 完成後の呼び出し

全必須工程を完了した商品は `READY` へ進み、注文管理画面上部の「完成しました」欄へ表示します。提供予定時刻は呼び出しを制限する時刻ではなく目安なので、完成済みなら予定時刻前でも「今すぐ呼び出す」を押せます。誤操作防止のため、既存どおり5秒間は取り消せます。
