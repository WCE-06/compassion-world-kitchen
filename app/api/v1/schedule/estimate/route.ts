...
import { drinkWorkMinutes, liveKitchenLoad, timingAdjustmentBuffers } from "@/lib/schedule-store";
...
export async function POST(request: NextRequest) {
  ...
  const [load,buffers]=await Promise.all([liveKitchenLoad(),timingAdjustmentBuffers()]),result = calculateSchedule(body, await drinkWorkMinutes(), load.activeFoodOrders,load.activeMicrowaveSeconds,load.fryerPreheated,buffers);
