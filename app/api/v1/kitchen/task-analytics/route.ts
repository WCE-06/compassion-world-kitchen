...
type MeasuredEvent=EventRow&{calls:string[];intervalSeconds:number;overrunSeconds:number};
...
export async function GET(request:NextRequest){
  ...
  const completed=events.filter(event=>event.action==="COMPLETE"),measured=completed.filter((event):event is MeasuredEvent=>event.overrunSeconds!==null&&event.intervalSeconds!==null),delayed=measured.filter(event=>event.overrunSeconds>60);
  ...
  const bottlenecks=[...groups.values()].map(group=>{const sampleCount=group.length,expectedSeconds=Math.round(group.reduce((sum,event)=>sum+event.expectedSeconds,0)/sampleCount),averageIntervalSeconds=Math.round(group.reduce((sum,event)=>sum+event.intervalSeconds,0)/sampleCount),averageOverrunSeconds=Math.round(group.reduce((sum,event)=>sum+event.overrunSeconds,0)/sampleCount),maxOverrunSeconds=Math.max(...group.map(event=>event.overrunSeconds)),suggestedSeconds=Math.max(expectedSeconds,Math.ceil((expectedSeconds+Math.max(0,averageOverrunSeconds))/60)*60);return{title:group[0].title,equipment:group[0].equipment,sampleCount,expectedSeconds,averageIntervalSeconds,averageOverrunSeconds,maxOverrunSeconds,suggestedMinutes:Math.max(1,Math.ceil(suggestedSeconds/60)),needsReview:averageOverrunSeconds>60,confidence:sampleCount>=3?"ENOUGH":"LOW"}}).sort((a,b)=>Number(b.needsReview)-Number(a.needsReview)||b.averageOverrunSeconds-a.averageOverrunSeconds||b.sampleCount-a.sampleCount).slice(0,8);
  return NextResponse.json({from,to:now,summary:{completed:completed.length,measured:measured.length,delayed:delayed.length,onTime:measured.length-delayed.length},bottlenecks,events:events.slice(-200).reverse()},{headers:{"Cache-Control":"no-store"}});
}
