...
export async function GET(request:NextRequest){
  ...
  return NextResponse.json({fryerPreheated:await fryerPreheated(),fryerTemperatureCelsius:200,fryerCookMinutes:4,fryerPreheatMinutes:10},{headers:{"Cache-Control":"no-store"}});
}
...
export async function PATCH(request:NextRequest){
  ...
  return NextResponse.json({fryerPreheated:await setFryerPreheated(body.fryerPreheated),fryerTemperatureCelsius:200,fryerCookMinutes:4,fryerPreheatMinutes:10});
}
