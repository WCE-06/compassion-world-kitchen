...
export default function KitchenBoard({ displayOnly = false }: { displayOnly?: boolean }) {
  ...
  return <main className={`board-shell ${screen === "CALL_MONITOR" ? "call-screen" : ""} ${displayOnly ? "display-only" : ""}`}>
    ...
    {screen === "MENU" ? <MenuManager /> : screen === "MASTER" ? <CookingMaster /> : screen === "HOURS" ? <BusinessHoursManager /> : screen === "TEST" ? <TestOrderPanel onCreated={()=>{setScreen("ORDERS");void load(true)}}/> : screen === "HISTORY" ? <section className="history-workspace"><div className="workspace-head"><div><p>FULFILLMENT HISTORY</p><h1>当日の受渡履歴</h1><small>誤操作した注文を呼出中へ戻し、顧客モニターへ再表示できます</small></div></div><div className="history-list">{history.length ? history.map((item) => <article key={item.id}><div className={`history-number ${item.department.toLowerCase()}`}><small>{item.department === "FOOD" ? "フード" : "ドリンク"}</small><strong>{String(item.callNumber).padStart(3, "0")}</strong></div><div><b>{item.items.map((entry) => `${entry.name} ×${entry.quantity}`).join("、")}</b><span>{new Date(item.updatedAt).toLocaleTimeString("ja-JP", { hour: "2-digit", minute: "2-digit" })}・{item.status === "PICKED_UP" ? "受渡完了" : "取消"}</span></div>{item.status === "PICKED_UP" && <button disabled={updating === item.id} onClick={() => void act(item, "RESTORE_CALL")}>呼出中へ戻して再呼出</button>}</article>) : <p className="history-empty">本日の受渡履歴はまだありません</p>}</div></section> : screen === "ANNOUNCEMENTS" ? <section className="announcement-workspace"><div className="workspace-head"><div><p>STORE ANNOUNCEMENTS</p><h1>館内放送</h1><small>放送前に上部の「音声・BGMを開始」を押してください</small></div></div><div className="announcement-grid">{[{title:"営業開始",tag:"OPEN",text:"おはようございます。本日も、Aozora Kitchenをご利用いただき、誠にありがとうございます。ただいまより営業を開始いたします。皆さまのご利用を心よりお待ちしております。"},{title:"ラストオーダー",tag:"LAST ORDER",text:"ご来館中のお客様にご案内いたします。Aozora Kitchenは、まもなくラストオーダーのお時間となります。ご注文予定のお客様は、お早めにご利用ください。"},{title:"営業終了",tag:"CLOSE",text:"ご来館中のお客様にご案内いたします。本日のAozora Kitchenの営業は終了いたしました。本日もご利用いただき、誠にありがとうございました。"}].map((item) => <article key={item.tag}><span>{item.tag}</span><h2>{item.title}</h2><p>{item.text}</p><button onClick={() => broadcast(item.title, item.text)}>▶ この放送を流す</button></article>)}</div></section> : screen === "CALL_MONITOR" ? <section className="customer-call-monitor">
      ...
      {department==="ALL"&&optimizedTasks.length>0&&<section className="parallel-task-board optimized"><header><div><b>最短工程ナビ</b><small>推奨順を表示しますが、完了した工程から自由に繰り上げ可能です。新しい注文は4秒以内に再計算します。</small></div><span>{optimizedTasks.filter(task=>!optimizerDone.has(task.id)).length}工程</span></header><div>{optimizedTasks.map((task,index)=>{const done=optimizerDone.has(task.id);return <article key={task.id} className={done?"done":""}><strong className="task-sequence">{index+1}</strong><div><small>{task.equipment}・{task.minutes>0?`所要 ${formatDuration(task.minutes*60)}`:"完了条件 180℃到達"}{task.calls.length?`・対象 ${task.calls.join(" / ")}`:""}</small><b>{task.title}</b><p>{task.detail}</p></div><button disabled={done} onClick={()=>setOptimizerDone(current=>new Set([...current,task.id]))}>{done?"完了済み":"完了 →"}</button></article>})}</div></section>}
...
function microwaveOperations(unit:Fulfillment){const name=unit.items[0]?.name??"";if(/カルボナーラパスタ/.test(name))return[{key:"pasta",title:"カルボナーラパスタの麺180gを加熱する",seconds:190,watts:600},{key:"sauce",title:"共通カルボナーラソースを加熱する",seconds:50,watts:600}];if(/濃厚魚介つけ麺|つけ麺/.test(name))return[{key:"noodle",title:"濃厚魚介つけ麺の麺を加熱する",seconds:180,watts:600},{key:"soup",title:"濃厚魚介つけ麺のつけ汁・角煮を加熱する",seconds:40,watts:600}];if(/贅沢ポテト/.test(name))return[{key:"sauce",title:"共通カルボナーラソースを加熱する",seconds:50,watts:600}];if(/ほうとう/.test(name))return[{key:"heat1",title:"ほうとうを加熱する（1回目）",seconds:300,watts:600},{key:"heat2",title:"ほうとうを混ぜて加熱する（2回目）",seconds:300,watts:600},{key:"heat3",title:"ほうとうを混ぜて最終加熱する",seconds:120,watts:600}];if(/うどん/.test(name))return[{key:"heat1",title:`${name}を加熱する（1回目）`,seconds:180,watts:1000},{key:"heat2",title:`${name}を混ぜて加熱する（2回目）`,seconds:180,watts:1000}];if(/角煮丼/.test(name))return[{key:"heat",title:"角煮丼を加熱する",seconds:70,watts:1000}];if(/煮カツ丼/.test(name))return[{key:"heat1",title:"煮カツ丼を加熱する（1回目）",seconds:78,watts:1000},{key:"heat2",title:"溶き卵を加えて再加熱する",seconds:30,watts:1000}];return[]}
...
function clarifyMicrowaveTask(task:OptimizedTask){let action="表示されたワット数と時間で加熱する。";if(/ほうとうを加熱する（1回目）/.test(task.title))action="600Wで5分加熱する。終了したら取り出し、底から全体をよく混ぜる。";else if(/ほうとうを混ぜて加熱する（2回目）/.test(task.title))action="混ぜ終えたほうとうを600Wで5分加熱する。終了したら再び全体をよく混ぜる。";else if(/ほうとうを混ぜて最終加熱/.test(task.title))action="混ぜ終えたほうとうを600Wで2分加熱する。終了後、温度と混ざり具合を確認する。";else if(/カルボナーラパスタの麺/.test(task.title))action="パスタ180gを600Wで3分10秒加熱する。終了後、麺をほぐす。";else if(/共通カルボナーラソース/.test(task.title))action="共通カルボナーラソースを600Wで50秒加熱する。終了後、袋の温度を確認する。";else if(/つけ麺の麺/.test(task.title))action="濃厚魚介つけ麺の麺を600Wで3分加熱する。終了後、水切り・水締めを行う。";else if(/つけ麺のつけ汁/.test(task.title))action="つけ汁・角煮を600Wで40秒加熱する。終了後、スープの素とお湯100ccを混ぜる。";else if(/うどん.*1回目/.test(task.title))action="1000Wで3分加熱する。終了したら取り出し、全体を混ぜる。";else if(/うどん.*2回目/.test(task.title))action="混ぜ終えたうどんを1000Wで3分加熱する。終了後、温度を確認する。";else if(/角煮丼/.test(task.title))action="角煮丼を1000Wで1分10秒加熱する。終了後、中心温度を確認する。";return{...task,detail:`${action} 推奨枠は${task.detail.match(/レンジ開始目安 ([^。]+)/)?.[1]??"今すぐ"}。レンジが予定より早く空いた場合は、待たずに繰り上げる。`}}
