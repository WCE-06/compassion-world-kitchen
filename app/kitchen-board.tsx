...
export default function KitchenBoard({ displayOnly = false }: { displayOnly?: boolean }) {
  ...
  const optimizedTasks=useMemo(()=>arrangeOptimizedTasks(optimizeKitchenV2([...data.FOOD,...data.DRINK].filter(item=>item.status==="ACCEPTED"||item.status==="COOKING"))),[data]);
  ...
  return <main className={`board-shell ${screen === "CALL_MONITOR" ? "call-screen" : ""} ${displayOnly ? "display-only" : ""}`}>
    ...
    {screen === "MENU" ? <MenuManager /> : screen === "MASTER" ? <CookingMaster /> : screen === "HOURS" ? <BusinessHoursManager /> : screen === "TEST" ? <TestOrderPanel onCreated={()=>{setScreen("ORDERS");void load(true)}}/> : screen === "HISTORY" ? <section className="history-workspace"><div className="workspace-head"><div><p>FULFILLMENT HISTORY</p><h1>当日の受渡履歴</h1><small>誤操作した注文を呼出中へ戻し、顧客モニターへ再表示できます</small></div></div><div className="history-list">{history.length ? history.map((item) => <article key={item.id}><div className={`history-number ${item.department.toLowerCase()}`}><small>{item.department === "FOOD" ? "フード" : "ドリンク"}</small><strong>{String(item.callNumber).padStart(3, "0")}</strong></div><div><b>{item.items.map((entry) => `${entry.name} ×${entry.quantity}`).join("、")}</b><span>{new Date(item.updatedAt).toLocaleTimeString("ja-JP", { hour: "2-digit", minute: "2-digit" })}・{item.status === "PICKED_UP" ? "受渡完了" : "取消"}</span></div>{item.status === "PICKED_UP" && <button disabled={updating === item.id} onClick={() => void act(item, "RESTORE_CALL")}>呼出中へ戻して再呼出</button>}</article>) : <p className="history-empty">本日の受渡履歴はまだありません</p>}</div></section> : screen === "ANNOUNCEMENTS" ? <section className="announcement-workspace"><div className="workspace-head"><div><p>STORE ANNOUNCEMENTS</p><h1>館内放送</h1><small>放送前に上部の「音声・BGMを開始」を押してください</small></div></div><div className="announcement-grid">{[{title:"営業開始",tag:"OPEN",text:"おはようございます。本日も、Aozora Kitchenをご利用いただき、誠にありがとうございます。ただいまより営業を開始いたします。皆さまのご利用を心よりお待ちしております。"},{title:"ラストオーダー",tag:"LAST ORDER",text:"ご来館中のお客様にご案内いたします。Aozora Kitchenは、まもなくラストオーダーのお時間となります。ご注文予定のお客様は、お早めにご利用ください。"},{title:"営業終了",tag:"CLOSE",text:"ご来館中のお客様にご案内いたします。本日のAozora Kitchenの営業は終了いたしました。本日もご利用いただき、誠にありがとうございました。"}].map((item) => <article key={item.tag}><span>{item.tag}</span><h2>{item.title}</h2><p>{item.text}</p><button onClick={() => broadcast(item.title, item.text)}>▶ この放送を流す</button></article>)}</div></section> : screen === "CALL_MONITOR" ? <section className="customer-call-monitor">
      ...
      {department==="ALL"&&optimizedTasks.length>0&&<section className="parallel-task-board optimized"><header><div><b>最短工程ナビ</b><small>判断に必要な作業だけを先に表示します。全工程は下で確認できます。</small></div><span>残り {optimizedTasks.filter(task=>!optimizerDone.has(task.id)).length}工程</span></header>{(()=>{const available=optimizedTasks.filter(task=>!optimizerDone.has(task.id)),focus=available[0],alternatives=available.slice(1,3);return focus?<aside className="decision-board"><div className={`focus-task ${equipmentClass(focus)}`}><small>いまやる作業</small><b>{focus.title}</b><p>{focus.detail}</p><button onClick={()=>setOptimizerDone(current=>new Set([...current,focus.id]))}>この作業を完了</button></div>{alternatives.length>0&&<div className="alternative-tasks"><small>手が空いたらできる作業</small>{alternatives.map(task=><button className={equipmentClass(task)} key={`available-${task.id}`} onClick={()=>document.getElementById(task.id)?.scrollIntoView({behavior:"smooth",block:"center"})}><b>{task.title}</b><span>{task.equipment}</span></button>)}</div>}</aside>:null})()}<details className="full-task-plan"><summary>全工程を見る</summary><div>{optimizedTasks.map((task,index)=>{const done=optimizerDone.has(task.id);return <article id={task.id} key={task.id} className={`${done?"done":""} ${equipmentClass(task)}`}><strong className="task-sequence">{index+1}</strong><div><small>{task.equipment}・{task.minutes>0?`所要 ${formatDuration(task.minutes*60)}`:"完了条件 180℃到達"}{task.calls.length?`・対象 ${task.calls.join(" / ")}`:""}</small><b>{task.title}</b><p>{task.detail}</p></div><button disabled={done} onClick={()=>setOptimizerDone(current=>new Set([...current,task.id]))}>{done?"完了済み":"完了 →"}</button></article>})}</div></details></section>}
...
function microwaveOperations(unit:Fulfillment){const name=unit.items[0]?.name??"";if(/カルボナーラパスタ/.test(name))return[{key:"pasta",title:"カルボナーラパスタの麺180gを加熱する",seconds:114,watts:1000},{key:"sauce",title:"共通カルボナーラソースを加熱する",seconds:30,watts:1000}];if(/濃厚魚介つけ麺|つけ麺/.test(name))return[{key:"noodle",title:"濃厚魚介つけ麺の麺を加熱する",seconds:108,watts:1000},{key:"soup",title:"濃厚魚介つけ麺のつけ汁・角煮を加熱する",seconds:24,watts:1000}];if(/贅沢ポテト/.test(name))return[{key:"sauce",title:"共通カルボナーラソースを加熱する",seconds:30,watts:1000}];if(/ほうとう/.test(name))return[{key:"heat1",title:"ほうとうを加熱する（1回目）",seconds:180,watts:1000},{key:"heat2",title:"ほうとうを混ぜて加熱する（2回目）",seconds:180,watts:1000},{key:"heat3",title:"ほうとうを混ぜて最終加熱する",seconds:72,watts:1000}];if(/うどん/.test(name))return[{key:"heat1",title:`${name}を加熱する（1回目）`,seconds:180,watts:1000},{key:"heat2",title:`${name}を混ぜて加熱する（2回目）`,seconds:180,watts:1000}];if(/角煮丼/.test(name))return[{key:"heat",title:"角煮丼を加熱する",seconds:70,watts:1000}];if(/煮カツ丼/.test(name))return[{key:"heat1",title:"煮カツ丼を加熱する（1回目）",seconds:78,watts:1000},{key:"heat2",title:"溶き卵を加えて再加熱する",seconds:30,watts:1000}];return[]}
...
function clarifyMicrowaveTask(task:OptimizedTask){let action="表示されたワット数と時間で加熱し、終了後に温度を確認する。";if(/ほうとうを加熱する（1回目）/.test(task.title))action="1000Wで3分加熱する。終了したら取り出し、底から全体をよく混ぜる。";else if(/ほうとうを混ぜて加熱する（2回目）/.test(task.title))action="混ぜ終えたほうとうを1000Wで3分加熱する。終了したら再び全体をよく混ぜる。";else if(/ほうとうを混ぜて最終加熱/.test(task.title))action="混ぜ終えたほうとうを1000Wで1分12秒加熱する。終了後、温度と混ざり具合を確認する。";else if(/カルボナーラパスタの麺/.test(task.title))action="パスタ180gを1000Wで1分54秒加熱する。終了後、麺をほぐす。";else if(/共通カルボナーラソース/.test(task.title))action="共通カルボナーラソースを1000Wで30秒加熱する。終了後、袋の温度を確認する。";else if(/つけ麺の麺/.test(task.title))action="濃厚魚介つけ麺の麺を1000Wで1分48秒加熱する。終了後、水切り・水締めを行う。";else if(/つけ麺のつけ汁/.test(task.title))action="つけ汁・角煮を1000Wで24秒加熱する。終了後、スープの素とお湯100ccを混ぜる。";else if(/うどん.*1回目/.test(task.title))action="1000Wで3分加熱する。終了したら取り出し、全体を混ぜる。";else if(/うどん.*2回目/.test(task.title))action="混ぜ終えたうどんを1000Wで3分加熱する。終了後、温度を確認する。";else if(/角煮丼/.test(task.title))action="角煮丼を1000Wで1分10秒加熱する。終了後、中心温度を確認する。";return{...task,detail:`${action} レンジが空いていれば、表示順を待たず今すぐ開始できる。`}}
...
function stepsForUnit(item:Fulfillment){if(item.department==="DRINK")return[`${item.items[0]?.name??"ドリンク"}を調製する`,`オプション・蓋・提供状態を確認する`];const product=item.items[0],name=product?.name??"商品";if(/カルボナーラパスタ/.test(name))return["パスタ180gを電子レンジ1000Wで1分54秒加熱する","共通カルボナーラソースを1000Wで30秒加熱する","パスタとソースを混ぜる","盛り付け・最終確認を行う"];return recipeFor(name,1,product?.options).steps}
