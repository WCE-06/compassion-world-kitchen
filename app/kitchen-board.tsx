...
type TaskProgress = { taskId:string; title:string; calls:string[]; completed:boolean; completedAt:number|null };
type AudioMaster = { deviceId:string; deviceName:string; leaseUntil:number; updatedAt:number };
type SyncIssue = { transactionId:string; transactionAt:number; terminalId:string|null; error:string|null; attemptCount:number; firstSeenAt:number };
type SyncHealth = { ok:boolean; delayedCount:number; issues:SyncIssue[]; lastAttemptAt:number|null; lastSuccessAt:number|null; lastError:string|null };
type ScheduleRisk = { level:"overdue"|"urgent"|"soon"|"normal"; label:string; remainingMs:number|null };
type ScheduleHistoryEntry = { calculatedAt:number; foodReadyAt:number|null; drinkReadyAt:number|null; reason:string|null; mode:string|null; calculationVersion:string|null };
...
export default function KitchenBoard({ displayOnly = false }: { displayOnly?: boolean }) {
  ...
  const [timingBuffers,setTimingBuffers]=useState<TimingBuffers>({});
  ...
  const [fryerPreheated,setFryerPreheatedState]=useState(false),[fryerUpdating,setFryerUpdating]=useState(false);
  const [deviceId,setDeviceId]=useState(""),deviceIdRef=useRef(""),deviceNameRef=useRef("厨房モニター");
  const [audioMaster,setAudioMaster]=useState<AudioMaster|null>(null),isAudioMasterRef=useRef(false);
  const [syncHealth,setSyncHealth]=useState<SyncHealth|null>(null),[retryingSync,setRetryingSync]=useState(false);
  const [taskUpdating,setTaskUpdating]=useState<string|null>(null);
  const [scheduleDialog,setScheduleDialog]=useState<ScheduleDialog|null>(null),[scheduleReason,setScheduleReason]=useState("混雑"),[scheduleUpdating,setScheduleUpdating]=useState(false),[scheduleMessage,setScheduleMessage]=useState("");
  const [scheduleHistory,setScheduleHistory]=useState<ScheduleHistory|null>(null),[scheduleHistoryLoading,setScheduleHistoryLoading]=useState(false);
  const optimizerSelection=useMemo(()=>{const pending=optimizedTasks.filter(task=>!optimizerDone.has(task.id)),priority=[...data.FOOD,...data.DRINK].filter(item=>item.status==="ACCEPTED"||item.status==="COOKING").map(item=>({item,risk:scheduleRisk(item)})).filter(row=>row.risk.level!=="normal").sort((a,b)=>(a.risk.remainingMs??Number.MAX_SAFE_INTEGER)-(b.risk.remainingMs??Number.MAX_SAFE_INTEGER))[0],priorityCall=priority?unitCall(priority.item):"",focus=pending.find(task=>task.id===optimizerFocus)??pending.find(task=>task.calls.includes(priorityCall))??pending[0];return{focus,priorityCall}},[optimizedTasks,optimizerDone,optimizerFocus,data]);

  const saveOptimizerTask=async(task:OptimizedTask,completed:boolean)=>{
  ...
  const recalculateAfterProgress=async(completedTask:OptimizedTask)=>{
    ...
    if(!affectedOrderIds.size)return;
    const remainingTasks=optimizedTasks.filter(task=>task.id!==completedTask.id&&!optimizerDone.has(task.id));
    const orders=[...affectedOrderIds].map(orderId=>{const units=active.filter(item=>item.orderId===orderId),calls=units.map(unitCall),related=remainingTasks.filter(task=>task.calls.some(call=>calls.includes(call))),byEquipment=(pattern:RegExp)=>related.filter(task=>pattern.test(task.equipment)).reduce((sum,task)=>sum+Math.max(0,task.minutes),0),microwave=byEquipment(/電子レンジ/),fryer=byEquipment(/フライヤー/)+(related.some(task=>/フライヤー/.test(task.equipment))&&!fryerPreheated?10:0),drink=byEquipment(/ドリンク/),prep=related.filter(task=>!/電子レンジ|フライヤー|ドリンク/.test(task.equipment)).reduce((sum,task)=>sum+Math.max(0,task.minutes),0),remainingMinutes=Math.max(2,microwave,fryer,drink,prep)+(related.length?2:0);return{orderId,remainingMinutes,foodReadyAt:maxReadyAt(units.filter(unit=>unit.department==="FOOD")),drinkReadyAt:maxReadyAt(units.filter(unit=>unit.department==="DRINK")),foodCallNumber:units.find(unit=>unit.department==="FOOD")?.callNumber??null,drinkCallNumber:units.find(unit=>unit.department==="DRINK")?.callNumber??null}});
    try{const response=await fetch("/api/v1/kitchen/schedule-recalculate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({orders,reason:`工程完了：${completedTask.title}`})});if(response.ok)await load(true)}catch{/* 工程完了は維持し、次回更新で再取得 */}
  };
  const completeOptimizerTask=(task:OptimizedTask)=>{
    setOptimizerDone(current=>new Set([...current,task.id]));
    setOptimizerHistory(current=>[...current.filter(id=>id!==task.id),task.id]);
    setOptimizerFocus("");
    void saveOptimizerTask(task,true);
  };
  const undoOptimizerTask=()=>{
    const historyIndex=optimizerHistory.findLastIndex(id=>optimizedTasks.some(task=>task.id===id));
    if(historyIndex<0)return;
    const taskId=optimizerHistory[historyIndex];
    const task=optimizedTasks.find(candidate=>candidate.id===taskId);
    if(!task)return;
    setOptimizerDone(current=>{const next=new Set(current);next.delete(taskId);return next});
    setOptimizerHistory(current=>current.filter((_,index)=>index!==historyIndex));
    setOptimizerFocus(taskId);
    void saveOptimizerTask(task,false);
  };
  ...
  async function enableAudio(force=false) {
    if(!deviceIdRef.current)return;
    try{
      const response=await fetch("/api/v1/kitchen/audio-master",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"CLAIM",deviceId:deviceIdRef.current,deviceName:deviceNameRef.current,force})});
      const body=await response.json() as {error?:string;master?:AudioMaster};
      if(response.status===409&&body.error==="AUDIO_MASTER_IN_USE"){
        setAudioMaster(body.master??null);isAudioMasterRef.current=false;
        setAudioStatus(`音声は「${body.master?.deviceName??"別端末"}」で再生中です`);return;
      }
      if(!response.ok)throw new Error(body.error??"音声担当を設定できませんでした");
      setDeviceId(deviceIdRef.current);
      setAudioMaster(body.master??null);isAudioMasterRef.current=true;
      await startAudioPlayback();
    }catch(error){setAudioStatus(error instanceof Error?friendly(error.message):"音声担当を設定できませんでした")}
  }

  async function takeOverAudio(){
    if(!window.confirm(`音声再生を「${audioMaster?.deviceName??"別端末"}」からこの端末へ切り替えますか？`))return;
  ...
  async function loadOperations(){
    try{
      const [progressResponse,healthResponse,masterResponse,timingResponse]=await Promise.all([fetch("/api/v1/kitchen/task-progress",{cache:"no-store"}),fetch("/api/v1/kitchen/sync-health",{cache:"no-store"}),fetch("/api/v1/kitchen/audio-master",{cache:"no-store"}),fetch("/api/v1/kitchen/timing-adjustments",{cache:"no-store"})]);
      const [progress,health,master,timing]=await Promise.all([progressResponse.json(),healthResponse.json(),masterResponse.json(),timingResponse.json()]) as [{progress?:TaskProgress[]},SyncHealth,{master?:AudioMaster|null},{buffers?:TimingBuffers}];
      if(progressResponse.ok){const completed=(progress.progress??[]).filter(row=>row.completed);setOptimizerDone(new Set(completed.map(row=>row.taskId)));setOptimizerHistory(completed.sort((a,b)=>(a.completedAt??0)-(b.completedAt??0)).map(row=>row.taskId))}
      if(healthResponse.ok)setSyncHealth(health);
      if(timingResponse.ok)setTimingBuffers(timing.buffers??{});
      if(masterResponse.ok){setDeviceId(deviceIdRef.current);const current=master.master??null;setAudioMaster(current);const owns=Boolean(current&&current.deviceId===deviceIdRef.current);isAudioMasterRef.current=owns;if(audioEnabledRef.current&&!owns){audioEnabledRef.current=false;setAudioEnabled(false);bgmRef.current?.pause();setAudioStatus(current?`音声担当が「${current.deviceName}」へ切り替わりました`:"音声担当の接続が切れました")}}
    }catch{/* 注文表示を止めず、次回更新で再試行 */}
  }
  async function retryOrderSync(){setRetryingSync(true);try{const response=await fetch("/api/v1/kitchen/sync-health",{method:"POST"}),body=await response.json() as SyncHealth&{error?:string};if(!response.ok)throw new Error(body.error??"再取得できませんでした");setSyncHealth(body);await load(true)}catch(error){setMessage(error instanceof Error?friendly(error.message):"注文を再取得できませんでした")}finally{setRetryingSync(false)}}
  async function toggleFryer(){setFryerUpdating(true);try{const response=await fetch("/api/v1/kitchen/equipment",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({fryerPreheated:!fryerPreheated})}),body=await response.json();if(!response.ok)throw new Error(body.error??"フライヤー状態を更新できませんでした");setFryerPreheatedState(Boolean(body.fryerPreheated));setMessage(body.fryerPreheated?"フライヤー稼働中：提供予定から予熱10分を除外します":"フライヤー停止中：揚げ物の提供予定へ予熱10分を加算します")}catch(error){setMessage(error instanceof Error?friendly(error.message):"フライヤー状態を更新できませんでした")}finally{setFryerUpdating(false)}}

  function broadcast(title: string, text: string) {
    if (!audioEnabledRef.current || !isAudioMasterRef.current) { setAudioStatus("館内放送は音声担当端末から実行してください"); return; }
    audioQueue.current = audioQueue.current.catch(()=>undefined).then(async () => {
  ...
  async function load(silent = false) {
    ...
    try {
      ...
      if (!displayOnly) {
        ...
        active.filter((item) => item.status === "ACCEPTED" && !knownUnits.current.has(item.id) && ordersInitialized.current).forEach((item) => freshOrders.set(item.orderId, [...(freshOrders.get(item.orderId) ?? []), item]));
      ...
      previousCalled.current = called; setData(next); if (foodHistoryResponse.ok && drinkHistoryResponse.ok) { const normalizedHistory=normalizeKitchenDepartments(foodHistory.units ?? [],drinkHistory.units ?? []); setHistory([...normalizedHistory.FOOD,...normalizedHistory.DRINK].sort((a,b)=>b.updatedAt-a.updatedAt)); } await Promise.all([loadEquipment(),loadOperations()]);setLastSync(new Date()); setMessage("");
  ...
  async function submitScheduleAdjustment(){
    ...
    const orderUnits=[...data.FOOD,...data.DRINK].filter(unit=>unit.orderId===scheduleDialog.item.orderId),payload={action:scheduleDialog.action,orderId:scheduleDialog.item.orderId,minutes:scheduleDialog.minutes,reason:scheduleDialog.action==="ADJUST"?scheduleReason:undefined,foodReadyAt:maxReadyAt(orderUnits.filter(unit=>unit.department==="FOOD")),drinkReadyAt:maxReadyAt(orderUnits.filter(unit=>unit.department==="DRINK")),foodCallNumber:orderUnits.find(unit=>unit.department==="FOOD")?.callNumber??null,drinkCallNumber:orderUnits.find(unit=>unit.department==="DRINK")?.callNumber??null};
    try{const response=await fetch("/api/v1/kitchen/schedule-adjust",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}),body=await response.json();if(!response.ok)throw new Error(body.error??"予定時刻を変更できませんでした");setScheduleDialog(null);await load(true);setScheduleMessage(scheduleDialog.action==="UNDO"?"直前の提供予定へ戻しました":`提供予定を${scheduleDialog.minutes}分延長しました`);window.setTimeout(()=>setScheduleMessage(""),5000)}catch(error){setScheduleMessage(error instanceof Error?friendly(error.message):"予定時刻を変更できませんでした")}finally{setScheduleUpdating(false)}
  }

  async function openScheduleHistory(item:Fulfillment){
    setScheduleDialog({item,action:"HISTORY"});setScheduleHistory(null);setScheduleHistoryLoading(true);setScheduleMessage("");
    try{const response=await fetch(`/api/v1/kitchen/schedule-adjust?orderId=${encodeURIComponent(item.orderId)}`,{cache:"no-store"}),body=await response.json();if(!response.ok)throw new Error(body.error??"変更履歴を取得できませんでした");setScheduleHistory(body as ScheduleHistory)}catch(error){setScheduleMessage(error instanceof Error?friendly(error.message):"変更履歴を取得できませんでした");setScheduleDialog(null)}finally{setScheduleHistoryLoading(false)}
  }
  ...
  return <main className={`board-shell screen-${screen.toLowerCase()} ${screen === "CALL_MONITOR" ? "call-screen" : ""} ${displayOnly ? "display-only" : ""}`}>
    ...
    {screen === "ANALYTICS" ? <TaskAnalytics/> : screen === "MENU" ? <MenuManager /> : screen === "MASTER" ? <CookingMaster /> : screen === "HOURS" ? <BusinessHoursManager /> : screen === "TEST" ? <TestOrderPanel onCreated={()=>{setScreen("ORDERS");void load(true)}}/> : screen === "HISTORY" ? <section className="history-workspace"><div className="workspace-head"><div><p>FULFILLMENT HISTORY</p><h1>当日の受渡履歴</h1><small>誤操作した注文を呼出中へ戻し、顧客モニターへ再表示できます</small></div></div><div className="history-list">{history.length ? history.map((item) => <article key={item.id}><div className={`history-number ${item.department.toLowerCase()}`}><small>{item.department === "FOOD" ? "フード" : "ドリンク"}</small><strong>{String(item.callNumber).padStart(3, "0")}</strong></div><div><b>{item.items.map((entry) => `${entry.name} ×${entry.quantity}`).join("、")}</b><span>{new Date(item.updatedAt).toLocaleTimeString("ja-JP", { hour: "2-digit", minute: "2-digit" })}・{item.status === "PICKED_UP" ? "受渡完了" : "取消"}</span></div>{item.status === "PICKED_UP" && <button disabled={updating === item.id} onClick={() => void act(item, "RESTORE_CALL")}>呼出中へ戻して再呼出</button>}</article>) : <p className="history-empty">本日の受渡履歴はまだありません</p>}</div></section> : screen === "ANNOUNCEMENTS" ? <section className="announcement-workspace"><div className="workspace-head"><div><p>STORE ANNOUNCEMENTS</p><h1>館内放送</h1><small>放送前に上部の「音声・BGMを開始」を押してください</small></div></div><div className="announcement-grid">{[{title:"営業開始",tag:"OPEN",text:"おはようございます。本日も、Aozora Kitchenをご利用いただき、誠にありがとうございます。ただいまより営業を開始いたします。皆さまのご利用を心よりお待ちしております。"},{title:"ラストオーダー",tag:"LAST ORDER",text:"ご来館中のお客様にご案内いたします。Aozora Kitchenは、まもなくラストオーダーのお時間となります。ご注文予定のお客様は、お早めにご利用ください。"},{title:"営業終了",tag:"CLOSE",text:"ご来館中のお客様にご案内いたします。本日のAozora Kitchenの営業は終了いたしました。本日もご利用いただき、誠にありがとうございました。"}].map((item) => <article key={item.tag}><span>{item.tag}</span><h2>{item.title}</h2><p>{item.text}</p><button onClick={() => broadcast(item.title, item.text)}>▶ この放送を流す</button></article>)}</div></section> : screen === "CALL_MONITOR" ? <section className="customer-call-monitor">
      ...
      {department==="ALL"&&scheduleAlerts.attention.length>0&&<section className={`schedule-alert-board ${scheduleAlerts.overdue?"has-overdue":""}`} role="status"><div><small>提供予定アラート</small><b>{scheduleAlerts.overdue?`提供予定を過ぎた注文が${scheduleAlerts.overdue}件あります`:`5分以内の注文が${scheduleAlerts.urgent}件あります`}</b></div>{scheduleAlerts.attention.slice(0,3).map(({item,risk},index)=><article key={item.id} className={`risk-${risk.level}`}><span>{index===0?"最優先":"次に確認"}</span><strong>{unitCall(item)}</strong><b>{item.items.map(product=>product.name).join("・")}</b><em>{risk.label}</em></article>)}</section>}
      {department==="ALL"&&optimizedTasks.length>0&&<section className="parallel-task-board optimized"><header><div><b>最短工程ナビ</b><small>工程の完了状況と実作業時間は全端末で共有され、再読み込み後も復元されます。</small></div><div className="optimizer-header-actions"><button disabled={optimizerHistory.length===0||Boolean(taskUpdating)} onClick={undoOptimizerTask}>↩ 一つ前の工程に戻る</button><span>残り {optimizedTasks.filter(task=>!optimizerDone.has(task.id)).length}工程</span></div></header>{(()=>{const pending=optimizedTasks.filter(task=>!optimizerDone.has(task.id)),priorityCall=optimizerSelection.priorityCall,focus=optimizerSelection.focus,remaining=pending.filter(task=>task.id!==focus?.id),drink=remaining.find(task=>equipmentClass(task)==="equipment-drink"),alternatives=[remaining[0],drink??remaining[1]].filter((task,index,array):task is OptimizedTask=>Boolean(task)&&array.indexOf(task)===index).slice(0,2),isPreheat=focus?.id.startsWith("fryer-preheat");return focus?<aside className="decision-board"><div className={`focus-task ${equipmentClass(focus)}`}><small>{priorityCall&&focus.calls.includes(priorityCall)?`最優先 ${priorityCall}・いまやる作業`:"いまやる作業・時間計測中"}</small><b>{focus.title}</b><p>{focus.detail}</p><button disabled={taskUpdating===focus.id} onClick={()=>completeOptimizerTask(focus)}>{taskUpdating===focus.id?"保存中…":isPreheat?"予熱を開始した":"この作業を完了"}</button></div>{alternatives.length>0&&<div className="alternative-tasks"><small>手が空いたらできる作業</small>{alternatives.map(task=><button className={equipmentClass(task)} key={`available-${task.id}`} onClick={()=>setOptimizerFocus(task.id)}><b>{task.title}</b><span>{task.equipment}・これを今やる</span></button>)}</div>}</aside>:null})()}<details className="full-task-plan"><summary>全工程を見る</summary><div>{optimizedTasks.map((task,index)=>{const done=optimizerDone.has(task.id),isPreheat=task.id.startsWith("fryer-preheat");return <article id={task.id} key={task.id} className={`${done?"done":""} ${equipmentClass(task)}`}><strong className="task-sequence">{index+1}</strong><div><small>{task.equipment}・{task.minutes>0?`所要 ${formatDuration(task.minutes*60)}`:"開始操作のみ"}{task.calls.length?`・対象 ${task.calls.join(" / ")}`:""}</small><b>{task.title}</b><p>{task.detail}</p></div><button disabled={done||taskUpdating===task.id} onClick={()=>completeOptimizerTask(task)}>{taskUpdating===task.id?"保存中…":done?(isPreheat?"開始済み":"完了済み"):(isPreheat?"予熱を開始した →":"完了 →")}</button></article>})}</div></details></section>}
      {message && <p className="form-notice error">{message}</p>}
      {scheduleMessage&&<p className="form-notice schedule-message">{scheduleMessage}</p>}
      <section className="order-grid" aria-live="polite">
        ...
        {current.map((item) => { const action = actionByStatus[item.status],risk=scheduleRisk(item); return <article className={`order-card status-${item.status.toLowerCase()} department-${item.department.toLowerCase()} risk-${risk.level}`} key={item.id}>
...
function unitCall(item:Fulfillment){return`${item.department==="FOOD"?"F":"D"}${String(item.callNumber).padStart(3,"0")}`}
function maxReadyAt(units:Fulfillment[]){const values=units.map(unit=>unit.estimatedReadyAt??unit.readyAt).filter((value):value is number=>typeof value==="number"&&value>0);return values.length?Math.max(...values):null}
function scheduleRisk(item:Fulfillment):ScheduleRisk{if(item.status!=="ACCEPTED"&&item.status!=="COOKING")return{level:"normal",label:"",remainingMs:null};const dueAt=item.estimatedReadyAt??item.readyAt;if(!dueAt)return{level:"normal",label:"",remainingMs:null};const remainingMs=dueAt-Date.now(),minutes=Math.max(1,Math.ceil(Math.abs(remainingMs)/60_000));if(remainingMs<=0)return{level:"overdue",label:`${minutes}分超過`,remainingMs};if(remainingMs<=5*60_000)return{level:"urgent",label:`残り${minutes}分`,remainingMs};if(remainingMs<=10*60_000)return{level:"soon",label:`残り${minutes}分`,remainingMs};return{level:"normal",label:"",remainingMs}}
...
function finalizeKitchenTasks(tasks:OptimizedTask[],units:Fulfillment[]){
  ...
  const riceUnits=units.filter(unit=>/丼|ご飯|ライス/.test(name(unit))),riceAssignments=riceUnits.map(unit=>`${call(unit)} ${name(unit)}：${riceGrams(name(unit),unit.items[0]?.options)}g`),kakuni=riceUnits.filter(unit=>/角煮丼/.test(name(unit)));
  const enriched=tasks.map(task=>{
    if(task.id.startsWith("fryer-preheat"))return{...task,title:"フライヤーをオンにして200℃の予熱を開始する",minutes:0,detail:"電源を入れて200℃に設定する。加熱が始まったことを確認したら、温度到達を待たずに「予熱を開始した」を押して次の作業へ進む。"};
    if(task.id.startsWith("fryer-load"))return{...task,detail:`温度表示が200℃に到達したことを確認してから投入する。${task.detail}`};
    if(task.id.startsWith("rice-prep"))return{...task,title:`商品別にご飯を先に盛る（合計${riceUnits.reduce((sum,unit)=>sum+riceGrams(name(unit),unit.items[0]?.options),0)}g）`,detail:`${riceAssignments.join(" ／ ")}。${kakuni.length?`角煮丼（${kakuni.map(call).join("・")}）は、ご飯の上に解凍済み角煮1袋・冷凍ネギ・天かすを載せ、蓋をせずレンジ待ちへ置く。`:""}`};
...
function riceGrams(productName:string,options?:string[]){const text=`${productName} ${options?.join(" ")??""}`;if(/ちょこっとライス|少なめ|150g/.test(text))return 150;if(/大盛ライス|大盛|250g/.test(text))return 250;return 200}
...
function dateTime(time:number){return new Intl.DateTimeFormat("ja-JP",{month:"numeric",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"}).format(new Date(time))}
function scheduleTimes(value:{foodReadyAt:number|null;drinkReadyAt:number|null}){const times=[value.foodReadyAt?`フード ${clock(value.foodReadyAt)}`:"",value.drinkReadyAt?`ドリンク ${clock(value.drinkReadyAt)}`:""].filter(Boolean);return times.join("・")||"できあがり次第"}
...
async function playLegacyCall(item: Fulfillment) {
  const number = legacyNumberFiles(item.callNumber);
  await playLegacyFiles([
    "chime_start.mp3",
    "complete_intro.mp3",
    ...number,
    "number_customer.mp3",
...
function legacyNumberFiles(callNumber:number){const number=Math.trunc(callNumber);if(number<=0||number>999)return[];const hundreds=Math.floor(number/100)*100,rest=number%100,files:string[]=[];if(hundreds)files.push(`${hundreds}.mp3`);if(rest)files.push(`${rest}.mp3`);return files}
function legacyVoiceUrl(file:string){return `${LEGACY_VOICE_BASE}/${encodeURIComponent(file)}`}
let legacyVoiceContext:AudioContext|null=null;
const legacyVoiceBuffers=new Map<string,AudioBuffer>();
