...
export default function KitchenBoard({ displayOnly = false }: { displayOnly?: boolean }) {
  ...
  const optimizedTasks=useMemo(()=>{const active=[...data.FOOD,...data.DRINK].filter(item=>item.status==="ACCEPTED"||item.status==="COOKING");return arrangeOptimizedTasks(expandDrinkTasks(optimizeKitchenV2(active),active))},[data]);
