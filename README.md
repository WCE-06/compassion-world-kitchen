...
メニュー管理はスマレジ・プラットフォームAPIのサンドボックスと接続します。`pos.products:read` と `pos.products:write` の最小権限を使い、商品・部門の取得、新規商品登録、商品情報修正を行います。クライアントIDとシークレットはホスティング環境の秘密設定だけに保存し、ブラウザへ公開しません。
