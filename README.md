...
flowchart LR
  ...
  Menu --> POS["セルフレジ"]
  Paygate["PAYGATE POS"] -->|"スマレジ確定取引"| Orders
...
メニュー管理はスマレジ・プラットフォームAPIと接続します。`pos.products:read` と `pos.products:write` を使い、商品・部門の取得、新規商品登録、商品情報修正を行います。PAYGATE POS連携では `pos.transactions:read` だけを追加し、確定取引を共通注文APIへ冪等登録します。商品コードが共通メニューに存在するフード・ドリンクだけをキッチンへ取り込み、取引取消も未受渡の注文へ反映します。クライアントIDとシークレットはホスティング環境の秘密設定だけに保存し、ブラウザへ公開しません。

必要な実行時設定:

- `SMAREGI_ENV`: `sandbox` または `production`
- `SMAREGI_CONTRACT_ID`
- `SMAREGI_CLIENT_ID`
...
- 決済済みPOS取引は24時間の再取得窓、2分の重複取得、ページング、冪等登録で取りこぼしを防ぐ。1件でも失敗した場合は取得位置を進めず再試行する
- 工程完了状態は共通DBへ保存し、再読み込み・再接続・別端末でも復元する
- 画面は4秒ごとに正本を再取得し、復帰・再接続・再フォーカス時は即時再取得する
- 音声担当はタブ単位のリースで1台に限定し、放送イベントも共通DBで一度だけ取得する
- 状態変更は5秒の取消猶予を設け、工程完了と受渡完了は履歴からも戻せる
- 顧客向け提供予定は器具競合、混雑、実測安全余裕に加えて受渡確認3分を含める
