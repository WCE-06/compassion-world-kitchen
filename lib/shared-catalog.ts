...
export async function getSharedCatalog() {
  ...
  let publication = new Map<string, { menuCategory: string; displaySequence: number; showOnSelfRegister: boolean; showOnMobileOrder: boolean }>();
  if (runtime.KITCHEN_API_TOKEN) {
    try {
      ...
      const publicationBody = await publicationResponse.json() as { products?: { productCode: string; menuCategory: string; displaySequence: number; showOnSelfRegister: boolean; showOnMobileOrder: boolean }[] };
